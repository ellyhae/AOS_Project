...
class FusionDenoiser(nn.Module):
    ...
    def __init__(self, fuse_scheme=0, img_size=128, swin_version='V1', window_size=8, use_checkpoint=False,
        ...
        if pretrained:
            ...
            self.fusion.load_state_dict(torch.load('snapshots/IFCNN-MAX.pth', map_location=device))
            
            if swin_version == 'V1' and swinir_grayscale:
                self.denoiser.load_state_dict(torch.load('snapshots/004_grayDN_DFWB_s128w8_SwinIR-M_noise15.pth', map_location=device)['params'])
            else:
                print('No pretrained weights available for grayscale Swin', swin_version)
